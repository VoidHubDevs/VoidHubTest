-- Wand UI loader + compatibility shim for legacy Window:Tab/Section API
...
if not ok or not Library then
    warn("[VoidHub] Failed to load Wand UI library. Ensure your executor allows HttpGet.")
    return
end

-- Create main Window using Wand UI
local Window = Library:MakeWindow({
    Title = "VoidHub | Blox Fruits",
    SubTitle = "dev by real_redz",
    ScriptFolder = "VoidHub-BloxFruits"
})

-- Compatibility layer: provide Window:Tab and Section:Toggle/Button/Dropdown/Slider/Label/Paragraph/Seperator
do
    local function makeTabProxy(tab)
        local proxy = {}
        function proxy:Section(title, pos)
            local section = tab:AddSection(title)
            local secproxy = {}
            function secproxy:Toggle(name, default, callback)
                section:AddToggle({ Name = name, Default = default or false, Callback = callback })
            end
            function secproxy:Button(name, callback)
                section:AddButton({ Name = name, Callback = callback })
            end
            function secproxy:Dropdown(name, options, default, callback)
                section:AddDropdown({ Name = name, Options = options or {}, Default = default, Callback = callback })
            end
            function secproxy:Slider(name, minmax, default, callback)
                -- minmax expected as table: {min, max, inc}
                local minv = 0; local maxv = 1; local inc = 0.01
                if type(minmax)=="table" then minv = minmax[1] or 0; maxv = minmax[2] or 1; inc = minmax[3] or 0.01 end
                section:AddSlider({ Name = name, Min = minv, Max = maxv, Increment = inc, Default = default or minv, Callback = callback })
            end
            function secproxy:Label(txt)
                section:AddParagraph(txt, "")
            end
            function secproxy:Paragraph(title, text)
                section:AddParagraph(title, text)
            end
            function secproxy:Seperator(text)
                section:AddParagraph(text or "", "")
            end
            return secproxy
        end
        return proxy
    ...
    function Window:Tab(title, icon)
        local t = Window:MakeTab({ Title = title, Icon = icon or "Home" })
        return makeTabProxy(t)
    end
end

-- Expose Window globally in case original script expects global Window
_G.Window = Window

-- End of shim


-- Begin merged original script (all 'Matsune' -> 'VoidHub')

local VoidHubA1 = {};

VoidHubA1["1"] = Instance.new("ScreenGui", game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"));
VoidHubA1["1"]["ZIndexBehavior"] = Enum.ZIndexBehavior.Sibling;

VoidHubA1["2"] = Instance.new("Frame", VoidHubA1["1"]);
VoidHubA1["2"]["BorderSizePixel"] = 0;
VoidHubA1["2"]["BackgroundColor3"] = Color3.fromRGB(0, 0, 0);
VoidHubA1["2"]["Size"] = UDim2.new(0, 50, 0, 50);
VoidHubA1["2"]["Draggable"] = true
VoidHubA1["2"]["Position"] = UDim2.new(0.20, 0, -0.1, 0);
VoidHubA1["2"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);


VoidHubA1["3"] = Instance.new("UICorner", VoidHubA1["2"]);
VoidHubA1["3"]["CornerRadius"] = UDim.new(1, 0);

VoidHubA1["4"] = Instance.new("TextButton", VoidHubA1["2"]);
VoidHubA1["4"]["TextWrapped"] = true;
VoidHubA1["4"]["BorderSizePixel"] = 0;
VoidHubA1["4"]["TextSize"] = 14;
VoidHubA1["4"]["TextColor3"] = Color3.fromRGB(255, 255, 255);
VoidHubA1["4"]["BackgroundColor3"] = Color3.fromRGB(255, 255, 255);
VoidHubA1["4"]["FontFace"] = Font.new([[rbxasset://fonts/families/SourceSansPro.json]], Enum.FontWeight.Bold, Enum.FontStyle.Normal);
VoidHubA1["4"]["Size"] = UDim2.new(0, 50, 0, 56);
VoidHubA1["4"]["BackgroundTransparency"] = 1;
VoidHubA1["4"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
VoidHubA1["4"]["Text"] = [[STOP TWEEN]];
VoidHubA1["4"]["Position"] = UDim2.new(0, 0, -0.07273, 0);
VoidHubA1["5"] = Instance.new("UIStroke", VoidHubA1["4"]);
VoidHubA1["5"]["Color"] = Color3.fromRGB(255, 0, 0);

VoidHubA1["6"] = Instance.new("UIStroke", VoidHubA1["2"]);
VoidHubA1["6"]["Color"] = Color3.fromRGB(169, 238, 8);

local function CancelTween()
    if _G.CancelTween then
        return
    end
...
VoidHubA1["4"].MouseButton1Click:Connect(function()
    CancelTween()
end)
...
_G.imageLogo = "rbxassetid://107753344956647"
_G.Logo = "rbxassetid://107753344956647"
_G.NameHub = "VoidHub | BloxFruits" -- ชื่อ Hub
_G.Title = "VoidHub Hub Script blox fruits" -- คำอธิบาย
-----------------------------------------------------------------
...
ImageButton.Image = "rbxassetid://107753344956647"
	...
	coroutine.resume(
		coroutine.create(
			function()
				...
				Circle.ZIndex = 10
				Circle.Image = "rbxassetid://107753344956647"
...
Disc_Logo.Image = "rbxassetid://107753344956647"
...
UICorner.Parent = ImageButton
	...
	function tabs:Tab(Name, icon)
		...
		ImageLabel.Image = "rbxassetid://107753344956647" .. icon
	   ...
	   Sep2.TextXAlignment = Enum.TextXAlignment.Center
		...
		function functionitem:Button(Name, callback)
			...
			function functionitem:Toggle(Name, default, callback)
				...
				MainToggle_3.Image = "rbxassetid://107753344956647"
				...
				if default then
					...
					TweenService:Create(
						...
						TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						{
							TextTransparency = 0efault and 0 or 0.4
						}
					):Play()
   ...
   spawn(function()
    ...
    spawn(function()
      ...
      AutoFarm:Toggle("Auto Farm Material", false,function(Value)
       ...
       CancelTween(_G.AutoFarmMaterial)
     end)
  ...
  Settings:Toggle("Dodge No CD", false,function(value)       
    ...
    while wait() do
     ...
     Items:Toggle("Auto Farm Elite Hunter", false,function(Values)      
          ...
          CancelTween(_G.AutoElitehunter)
       end)
...
spawn(function()
  ...
  Volcano:Toggle("Collect Bone", false, function(Value)
     ...
     CancelTween(_G.AutoCollectBone)
 end)
  ...
  Teleport:Toggle("Auto Tween To Island", false, function(value)
   ...
   Teleport:Dropdown("Select NPC",{"Blox Fruits Dealer","Remove Devil Fruit","Horned Man","Hungey Man","Previous Hero","Butler","Lunoven","Trevor","Elite Hunter","Player Hunter","Uzoth"},{"Random Devil Fruit"},function(value)
    ...
    Playersss:Toggle("Auto Kill Player Quest", false, function(Killzps)
        ...
        CancelTween(_G.AutoPlayerHunter)
	end)
